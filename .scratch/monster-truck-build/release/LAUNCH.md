# licensed — revised monster-truck playtest

Exactly three friends, each on their own computer and Steam account, must use the same release. Older builds can still appear at reception: the current discovery filter does not separate game versions. Compare source_commit and source_manifest_sha256 in MANIFEST.json before joining. Steam must be running and signed in; this development build uses app 480, so Steam may show Spacewar. No steam_appid.txt is needed.
...
Use the reception desk to gather your friends. All three pick the monster truck at the booking board, then sit in the waiting-room chairs. Walk with WASD, look with the mouse, use E to interact. Hold Space while walking against the truck to climb, aim at a highlighted seat, and press E to sit. E also leaves the seat. Steering uses A/D; speed uses W forward and S brake then reverse; balance uses A/D left/right and W/S forward/back. Release speed controls to slow. Stop and press 1/2/3 to request steering/speed/balance; the other occupant accepts with Y or declines with N. When prompted, hold R to recover. Escape opens mic/quit controls and the three-person concession proposal. Exactly three humans remain required; there is no solo or two-person test.

This revised cooperation checkpoint has an open-cab monster truck, a shared overhead camera, and a complete short test: turn, bumps, then parking. It can award a shared pass and rating, including after recovery or cone contacts. Durable personal licenses and the longer route remain deferred. SESSION.md describes the next human playtest. Windows native launch, Intel Mac launch and mixed-platform three-person Steam play remain human verification tasks. Local automated checks and an Apple silicon startup do not establish those results.
