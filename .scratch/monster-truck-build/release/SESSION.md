...
Status: prepared; no human session has occurred. Use exactly three humans including the user, each on a separate Windows x86_64 or macOS (Apple silicon/Intel) computer with a different logged-in Steam account. The user moved truck/learner production 07–08 before this session. Use the cross-platform scrapyard release and compare the source commit in each MANIFEST.json; both platform archives must come from the same release. The checkpoint-06 and checkpoint-06-modeled ZIPs are historical. SHA256.txt beside the release downloads identifies each platform archive. Older rooms remain discoverable, so confirm versions before joining. This remains the cooperation exercise rather than the complete driving test. All handling, ejection, rescue and six-second aftermath tuning remains provisional. No player can pass the full vehicle yet: checkpoint success below means completing the comparison exercise, not earning a license.

## Build and capture

Use MANIFEST.json inside each extracted bundle and verify the platform ZIP against the downloaded SHA256.txt. Record manifest/source digest, Godot version, GodotSteam version, operating-system version and architecture, CPU/GPU, display resolution/refresh, VSync, frame cap, wired/Wi-Fi connection, host, pseudonymous A/B/C identifiers and UTC start/end. Use the same build/settings for comparisons. Begin at 1280×720 windowed, Forward+, 60 FPS cap, existing 60 Hz physics; record any necessary machine-specific deviation. Keep Steam running. Launch with diagnostics using the platform-specific commands in README.md (`run-playtest.cmd` on Windows or the app executable in Terminal on macOS). No ENet flag. App 480 is development Steam transport, not owned-app acceptance. Invite/join through reception; all pick monster truck and sit. Steam failure screen or absent invite/join is a blocker to investigate, never permission to substitute ENet.

Capture all three screens with visible keystrokes (external recorder/keyboard view; avoid an omniscient game overlay) and voice with the players' consent. Each person says their identifier and a shared countdown before the first action. Retain the automatic `checkpoint-*.jsonl` files in `%APPDATA%\Godot\app_userdata\licensed` on Windows or `~/Library/Application Support/Godot/app_userdata/licensed/` on macOS; archive immediately after each host block. The launch log is `playtest-engine.log` in the extracted folder. Record the scenario ID aloud. Logs use monotonic local timestamps, so align them with video by the first boarding/control action, not by assuming synchronized clocks. Do not put real account identifiers into a public report.
